...
import type { Location, User, UOM } from "@/types";
...
export default function Scope3SetupPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [activityUoms, setActivityUoms] = useState<UOM[]>([]);
  ...
  const [editingAssignment, setEditingAssignment] = useState<Scope3Assignment | null>(null);
  ...
  const fetchFactorSets = async () => {
    ...
    try {
      const [libRes, compRes, catRes, uomRes] = await Promise.all([
      ...
      // Exclude emission UOMs — those are in the Emission Unit dropdown (kgCO2e / tCO2e)
      setActivityUoms((uomRes.data as UOM[]).filter((u) => u.category !== "emission"));
    } catch (err: any) {
  ...
  const handleSaveItem = async (data: Record<string, any>) => {
    ...
    try {
      const payload = {
        ...
        emission_unit: data.emission_unit || "kgCO2e",
        activity_unit: data.activity_unit || null,
        calc_method: data.calc_method || "AVERAGE_DATA",
        wtt_factor: data.wtt_factor ? Number(data.wtt_factor) : null,
  ...
  const handleSaveAssignment = async (data: {
    ...
    try {
      if (editingAssignment) {
        await tenantApi.updateScope3Assignment(editingAssignment.assignment_id, {
          assigned_to: data.assigned_to,
          factor_set_id: data.factor_set_id,
          entry_type: data.entry_type,
        });
        toast.success("Assignment updated");
      } else {
        // Create one assignment per selected category
        for (const catId of data.category_ids) {
          await tenantApi.createScope3Assignment({
            ghg_category_id: catId,
        ...
        toast.success(
          data.category_ids.length > 1
            ? `${data.category_ids.length} assignments created`
            : "Assignment created"
        );
      }
      ...
      toast.error(getApiError(err, editingAssignment ? "Failed to update assignment" : "Failed to create assignment"));
    } finally {
  ...
  const tabs: { key: Tab; label: string; icon: any; hidden?: boolean }[] = [
    { key: "factors", label: "Factor Sets", icon: Library },
    ...
    <>
      ...
      {tab === "assignments" && (
        <AssignmentsTab
          ...
          isAdmin={isAdmin}
          onCreateOpen={() => { setEditingAssignment(null); setAssignFormOpen(true); }}
          onEdit={(a) => { setEditingAssignment(a); setAssignFormOpen(true); }}
      ...
      {assignFormOpen && (
        <AssignmentFormModal
          ...
          onClose={() => { setAssignFormOpen(false); setEditingAssignment(null); }}
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold border-b border-[hsl(var(--border-hairline))]">
                      ...
                      <th className="text-right py-1.5 px-2">WTT</th>
...
function FactorItemFormModal({ open, editing, saving, categories, activityUoms, onClose, onSave }: {
  open: boolean;
  ...
  const [form, setForm] = useState({
    ...
    calc_method:      editing?.calc_method    || "AVERAGE_DATA",
    ...
    emission_unit:    editing?.emission_unit  || "kgCO2e",
    activity_unit:    editing?.activity_unit  || "",
    wtt_factor:       editing?.wtt_factor != null ? String(editing.wtt_factor) : "",
    notes:            editing?.notes          || "",
  });

  const isSpendBased = form.calc_method === "SPEND_BASED";
  const PHYSICAL_CATEGORIES = ["mass", "distance", "volume", "energy", "other"] as const;
  const SPEND_CATEGORIES    = ["currency"] as const;
  const unitCategories      = isSpendBased ? SPEND_CATEGORIES : PHYSICAL_CATEGORIES;
  ...
  const wttNum    = parseFloat(form.wtt_factor);
  const totalFactor = factorNum + (isNaN(wttNum) ? 0 : wttNum);
  ...
  const actUnitLabel = activityUoms.find((u) => u.symbol === form.activity_unit)?.display_name || form.activity_unit || "quantity";
        ...
        <SheetBody className="space-y-5">
          ...
          {!isNaN(factorNum) && factorNum > 0 && form.activity_unit && (
            <div className="mt-4 bg-accent border border-accent-foreground/20 rounded-md px-3 py-2.5">
              ...
              {!isNaN(wttNum) && wttNum > 0 && (
                <p className="text-[11px] text-violet-500 mt-0.5">
                  Direct: {factorNum} + WTT: {wttNum} = {totalFactor.toFixed(6)} {form.emission_unit} / {form.activity_unit}
                </p>
              )}
            </div>
          )}
...
/** GHG Protocol calculation method display names */
...
function AssignmentsTab({ assignments, isAdmin, onCreateOpen, onEdit, onDelete }: {
  assignments: Scope3Assignment[];
  isAdmin: boolean;
  onCreateOpen: () => void;
  onEdit: (a: Scope3Assignment) => void;
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold border-b border-border bg-sunken/50">
                ...
                <th className="text-left py-2 px-4">Calc. Methods</th>
...
/** GHG Protocol calculation methods for assignment configuration */
const ALL_CALC_METHODS = [
  {
    value: "SUPPLIER_SPECIFIC",
    label: "Supplier-specific",
    hint: "Qty × EF from supplier's own GHG inventory",
  },
  {
    value: "AVERAGE_DATA",
    label: "Average-data",
    hint: "Qty × EF from process LCA database",
  },
  {
    value: "SPEND_BASED",
    label: "Spend-based",
  ...
  },
  {
    ...
    hint: "Enter tCO₂e directly (e.g. from an LCA study)",
  },
];

function AssignmentFormModal({
  open, editing, categories, locations, users, companySets, saving, onClose, onSave,
}: {
  open: boolean;
  editing: Scope3Assignment | null;
  categories: Scope3GHGCategory[];
  locations: Location[];
  users: User[];
  companySets: Scope3FactorSet[];
  ...
  const [selectedCategoryIds, setSelectedCategoryIds] = useState<number[]>(
    isEdit ? [editing!.ghg_category_id] : []
  );
  const [locationId, setLocationId] = useState<string>(editing?.location_id ?? "");
  const [assignedTo, setAssignedTo] = useState<string>(editing?.assigned_to ?? "");
  const [factorSetId, setFactorSetId] = useState<string>(editing?.factor_set_id ?? "");
  const [entryTypes, setEntryTypes] = useState<string[]>(
    isEdit
      ? (editing!.entry_type || "SUPPLIER_SPECIFIC,AVERAGE_DATA").split(",").map((t) => t.trim()).filter(Boolean)
      : ["SUPPLIER_SPECIFIC", "AVERAGE_DATA"]  // default: both quantity-based methods
  );
  ...
  const toggleEntryType = (val: string) => {
    setEntryTypes((prev) =>
  ...
  const handleSubmit = () => {
    if (!isEdit && selectedCategoryIds.length === 0) return;
    if (entryTypes.length === 0) return;
    onSave({
      category_ids: isEdit ? [editing!.ghg_category_id] : selectedCategoryIds,
      location_id: locationId || null,
      assigned_to: assignedTo || null,
      factor_set_id: factorSetId || null,
      entry_type: entryTypes.join(","),
    });
  };
      ...
      <SheetContent size="wide">
        ...
        <SheetBody className="space-y-5">
          ...
          <div className="mt-4">
            ...
            <div className="space-y-2">
              {ALL_CALC_METHODS.map((m) => (
                <label key={m.value} className="flex items-start gap-2.5 cursor-pointer select-none group">
                  <input
                    type="checkbox"
